...
function parseArgs(argv) {
  ...
  for (let i = 0; i < args.length; i++) {
    if (args[i] === '--week') {
      targetDate = { type: 'week', value: parseInt(args[++i]) };
    } else if (args[i] === '--next') {
      targetDate = { type: 'next' };
    } else if (args[i] === '--date') {
      targetDate = { type: 'date', value: args[++i] };
    } else if (args[i] === '--adds-used') {
      addsUsed = parseInt(args[++i]);
...
function skaterQuality(stats, gp) {
  if (!gp || gp <= 0) return 0;
  const perGame = (
    (stats[STAT_IDS.G] || 0) * 3 +
    (stats[STAT_IDS.A] || 0) * 2 +
    (stats[STAT_IDS.SOG] || 0) * 0.3 +
    (stats[STAT_IDS.HIT] || 0) * 0.3 +
    (stats[STAT_IDS.BLK] || 0) * 0.3
  ) / gp;
  return perGame;
}

function goalieQuality(stats, gp) {
  if (!gp || gp <= 0) return 0;
  const perGame = (
    (stats[STAT_IDS.W] || 0) * 5 +
    (stats[STAT_IDS.SV] || 0) * 0.1
  ) / gp;
  const svpBonus = Math.max(0, ((stats[STAT_IDS.SVP] || 0) - 0.900)) * 200;
  return perGame + svpBonus;
}

function playerQuality(player) {
  const stats = player.stats || {};
  const gp = stats['0'] || player.gamesPlayed || 0;
  if (player.isGoalie) return goalieQuality(stats, gp);
  return skaterQuality(stats, gp);
}

async function fetchRosterStats(leagueKey, players) {
...
async function fetchMatchupNeeds(teamKey, leagueKey, week, settings) {
  ...
  try {
    ...
    const categories = [];
    const STAT_NAMES = {
      '1': 'G', '2': 'A', '3': 'P', '14': 'SOG', '31': 'HIT', '32': 'BLK',
      '19': 'W', '23': 'GAA', '26': 'SV%',
    };
    const LOWER_IS_BETTER = new Set(['23']);

    for (const sw of matchup.stat_winners) {
      const w = sw.stat_winner;
      const statId = w.stat_id;
...
function buildCategoryWeights(categoryNeeds) {
  if (!categoryNeeds) return null;

  const weights = {};
  for (const cat of categoryNeeds) {
    if (!['G', 'A', 'P', 'SOG', 'HIT', 'BLK'].includes(cat.name)) continue;
    const statId = cat.statId;
    if (cat.status === 'losing') {
      weights[statId] = 2.0 + Math.min(1.0, Math.abs(cat.gap));
    } else if (cat.status === 'tied') {
      weights[statId] = 1.5;
    } else {
      weights[statId] = cat.gap > 0.3 ? 0.5 : 1.0;
    }
  }
  return weights;
}

function weightedQuality(stats, gp, categoryWeights) {
  if (!gp || gp <= 0) return 0;
  if (!categoryWeights) {
    return (
      (stats[STAT_IDS.G] || 0) * 3 +
  ...
  return (
    (stats[STAT_IDS.G] || 0) * (categoryWeights['1'] || 1) * 3 +
    (stats[STAT_IDS.A] || 0) * (categoryWeights['2'] || 1) * 2 +
    (stats[STAT_IDS.SOG] || 0) * (categoryWeights['14'] || 1) * 0.3 +
    (stats[STAT_IDS.HIT] || 0) * (categoryWeights['31'] || 1) * 0.3 +
    (stats[STAT_IDS.BLK] || 0) * (categoryWeights['32'] || 1) * 0.3
  ) / gp;
}

// ── Streamer Scoring ────────────────────────────────────────

function isBangerLeague(settings) {
  const cats = settings.settings.stat_categories.stats;
  const names = cats
    .filter(c => c.stat.is_only_display_stat !== '1')
    .map(c => c.stat.display_name);
  return names.includes('HIT') || names.includes('BLK');
}

function scoringQualityPerGame(stats, gp) {
  if (!gp || gp <= 0) return 0;
  return (
    (stats[STAT_IDS.G] || 0) * 4 +
    (stats[STAT_IDS.A] || 0) * 3 +
    (stats[STAT_IDS.SOG] || 0) * 0.4
  ) / gp;
}

function bangerQualityRaw(stats) {
  return (
    (stats[STAT_IDS.HIT] || 0) * 1.5 +
    (stats[STAT_IDS.BLK] || 0) * 1.5 +
    (stats[STAT_IDS.SOG] || 0) * 0.3 +
    (stats[STAT_IDS.G] || 0) * 0.5 +
    (stats[STAT_IDS.A] || 0) * 0.3
  );
}

function scoreStreamers(freeAgents, days, teamGameDays, categoryWeights) {
  ...
  const enriched = freeAgents.map(fa => {
    ...
    fa.scoringQuality = scoringQualityPerGame(fa.stats || {}, effectiveGp);
    fa.bangerQuality = bangerQualityRaw(fa.stats || {});

    fa.quality = categoryWeights
      ? weightedQuality(fa.stats || {}, effectiveGp, categoryWeights)
      : playerQuality(fa);
...
async function run(config, cliArgs) {
  ...
  await fetchRosterStats(leagueKey, rosterPlayers);
  ...
  const banger = isBangerLeague(settings);
  ...
  if (projectedAdds.length > 0) {
    ...
    for (const addSpec of projectedAdds) {
      ...
      const match = allFreeAgents.find(fa =>
      ...
      if (match) {
        ...
        match.quality = playerQuality(match);
  ...
  if (currentWeek && isCurrentWeek) {
    ...
    categoryNeeds = await fetchMatchupNeeds(teamKey, leagueKey, currentWeek, settings);
    categoryWeights = buildCategoryWeights(categoryNeeds);
  }

  const scoredStreamers = scoreStreamers(allFreeAgents, days, teamGameDays, categoryWeights);
