...
const { resolveLeague, loadCachedSettings, saveCachedSettings } = require('./config');
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < args.length; i++) {
    if (args[i] === '--week') {
      ...
      noMatchup = true;
    } else if (args[i] === '--refresh-settings') {
  ...
  return { leagueKey: parsedLeagueKey, targetDate, addsUsed, addsUsedExplicit, includeGoalies, noMatchup, refreshSettings, manualBoosts, projectedAdds };
}
...
async function loadLeagueSettings(leagueKey, forceRefresh = false) {
  if (!forceRefresh) {
    const cached = loadCachedSettings(leagueKey);
    if (cached) return cached;
  }
  const settings = await client.getLeagueSettings(leagueKey);
  saveCachedSettings(leagueKey, settings);
  return settings;
}
...
/**
 * Find drop candidates factoring in actual lineup contribution.
 * A player with 4 games all on busy nights (benched every time) is a better
 * drop target than one with 3 games who fills a slot each time.
 */
function findDropCandidates(rosterPlayers, teamGameDays, days, excludeKeys = new Set()) {
  ...
  const enriched = rosterPlayers.map(p => {
    ...
    // Count how many future days this player actually starts vs sits
    let starts = 0;
    let benched = 0;
    for (const day of futureDays) {
      const isPlaying = day.playingPlayers.some(pp => pp.playerKey === p.playerKey);
      if (!isPlaying) continue;
      const isBenched = day.benched.some(bp => bp.playerKey === p.playerKey);
      if (isBenched) benched++;
      else starts++;
    }

    // Drop value: lower = better drop candidate
    // Quality contributes, but actual starts matter more
    // A benched player's quality is wasted — they add 0 to your lineup
    const dropValue = Math.max(0, p.quality || 0) * (starts || 0.1);

    return { ...p, futureGames, gamesRemaining: futureGames.length, starts, benched, dropValue };
  });

  // Sort by drop value (lowest first = best drop candidates)
  enriched.sort((a, b) => a.dropValue - b.dropValue);

  const skaters = enriched.filter(p => !p.isGoalie && !p.isIR && !excludeKeys.has(p.playerKey));
  if (skaters.length === 0) return [];

  // Use median dropValue as threshold — anyone below median is a candidate
  const sorted = [...skaters].sort((a, b) => a.dropValue - b.dropValue);
  const medianValue = sorted[Math.floor(sorted.length / 2)].dropValue;

  // Include players at or below median, plus anyone fully benched regardless
  const candidates = skaters.filter(p =>
    p.dropValue <= medianValue || (p.gamesRemaining > 0 && p.starts === 0)
  );
...
/**
 * Print teams ranked by streaming value: off-night games, total games,
 * and which days they play. Helps users search the waiver wire by team.
 */
function printTeamTargets(days, teamGameDays) {
  const futureDays = days.filter(d => !d.isPast);
  if (futureDays.length === 0) return;

  console.log('══ TEAM TARGETS (best streaming schedules) ═══════════════════');

  // Build team schedule summaries
  const teamStats = new Map();
  for (const [team, gameDays] of teamGameDays) {
    const futureGames = gameDays.filter(d => futureDays.some(fd => fd.date === d));
    if (futureGames.length === 0) continue;

    let offNightGames = 0;
    const dayLabels = [];
    for (const date of futureGames) {
      const day = futureDays.find(d => d.date === date);
      if (!day) continue;
      const label = formatDate(date).split(' ')[0]; // Mon, Tue, etc.
      dayLabels.push(day.isOffNight ? `${label}★` : label);
      if (day.isOffNight) offNightGames++;
    }

    teamStats.set(team, {
      team,
      totalGames: futureGames.length,
      offNightGames,
      dayLabels,
    });
  }

  // Sort: most off-night games first, then total games
  const sorted = [...teamStats.values()]
    .sort((a, b) => b.offNightGames - a.offNightGames || b.totalGames - a.totalGames);

  // Show at least 5, then include any team tied with the 5th on off-night count
  const MIN_TEAMS = 5;
  let cutoff = Math.min(MIN_TEAMS, sorted.length);
  if (cutoff < sorted.length) {
    const tiedWith = sorted[cutoff - 1].offNightGames;
    while (cutoff < sorted.length && sorted[cutoff].offNightGames === tiedWith) {
      cutoff++;
    }
  }
  const ranked = sorted.slice(0, cutoff);

  console.log('Team  Games  Off-Night  Schedule');
  console.log('─────────────────────────────────────────────────────────────');
  for (const t of ranked) {
    const offTag = t.offNightGames > 0 ? `${t.offNightGames} ★` : '—';
    console.log(`${t.team.padEnd(5)} ${String(t.totalGames).padStart(5)}  ${offTag.padStart(9)}  ${t.dayLabels.join('/')}`);
  }
  console.log();
}
...
function printDropCandidates(candidates) {
  console.log('══ DROP CANDIDATES (lowest lineup value) ══════════════════════');
  ...
  for (const p of candidates) {
    let lineupInfo;
    if (p.gamesRemaining === 0) {
      lineupInfo = 'no games remaining';
    } else if (p.starts === 0) {
      lineupInfo = `benched all ${p.benched} games`;
    } else if (p.benched > 0) {
      lineupInfo = `${p.starts} starts, ${p.benched} benched`;
    } else {
      lineupInfo = `${p.starts} starts`;
    }
    const statLine = formatStatLine(p, true);
    console.log(`  ${p.name} (${p.nhlTeam}) ${p.displayPosition} — ${p.gamesRemaining} games (${lineupInfo}) | Season: ${statLine || 'n/a'}`);
  }
  console.log();
}
...
async function run(config, cliArgs) {
  ...
  let { targetDate, addsUsed, addsUsedExplicit, includeGoalies, noMatchup, refreshSettings, manualBoosts, projectedAdds } = parsed;

  console.log('Loading league settings...');
  const settings = await loadLeagueSettings(leagueKey, refreshSettings);
  ...
  const addedKeys = new Set(projectedAddPlayers.map(p => p.playerKey));
  const dropCandidates = findDropCandidates(rosterPlayers, teamGameDays, days, addedKeys);
  ...
  printTeamTargets(days, teamGameDays);
